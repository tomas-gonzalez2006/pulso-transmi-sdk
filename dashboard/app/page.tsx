...
type Dashboard = {
  ...
  connection: { pulso_key_configured: boolean; supabase_configured: boolean; leaderboard_loaded: boolean; identity_loaded: boolean };
  leaderboard?: unknown; accuracy: number | null; coverage: number | null; drift: number | null;
  drift_detail: { recent_mean: number | null; baseline_mean: number | null; recent_count: number; baseline_count: number };
  runs: Array<Record<string, unknown>>; metrics: Array<Record<string, unknown>>; prediction_count: number; evaluated_count: number;
  latest_run?: Record<string, unknown> | null; errors_by_station: Record<string, { actual: number[]; predicted: number[] }>;
};
...
export default function Home() {
  ...
  return <main className="shell">
    ...
    {(!data.connection.pulso_key_configured || !data.connection.supabase_configured) && <div className="alert">Conexión incompleta: configura `PULSO_API_KEY` y `SUPABASE_SERVICE_ROLE_KEY` como variables privadas de Production en Vercel.</div>}
    <section className="grid"><Panel title="Drift de demanda" caption="Cambio relativo de la media de observaciones recientes frente a la ventana anterior."><div className="bars"><div><span>Anterior</span><i style={{ height: `${Math.min(100, (data.drift_detail.baseline_mean ?? 0) / Math.max(data.drift_detail.baseline_mean ?? 1, data.drift_detail.recent_mean ?? 1) * 100)}%` }} /><b>{number(data.drift_detail.baseline_mean, 0)}</b></div><div><span>Últimos 7 días</span><i className="accent" style={{ height: `${Math.min(100, (data.drift_detail.recent_mean ?? 0) / Math.max(data.drift_detail.baseline_mean ?? 1, data.drift_detail.recent_mean ?? 1) * 100)}%` }} /><b>{number(data.drift_detail.recent_mean, 0)}</b></div></div><p className="muted">{data.drift_detail.recent_count} observaciones recientes · {data.drift_detail.baseline_count} de referencia</p></Panel><Panel title="Accuracy por estación" caption="Calculada con WAPE cuando existe valor real evaluado."><div className="table">{stations.length ? stations.map((row) => <div className="row" key={row.station}><span>{row.station}</span><span className={row.accuracy !== null && row.accuracy < 88 ? "warn" : "goodText"}>{percent(row.accuracy)}</span></div>) : <p className="muted">Todavía no hay predicciones evaluadas.</p>}</div></Panel></section>
    <section className="grid"><Panel title="Leaderboard" caption="Fuente oficial protegida del API Pulso TransMi."><pre>{JSON.stringify(data.leaderboard ?? { message: "Sin datos disponibles" }, null, 2)}</pre></Panel><Panel title="Salud del pipeline" caption="Últimas ejecuciones persistidas en Supabase."><div className="table">{data.runs.length ? data.runs.slice(0, 8).map((run, i) => <div className="row" key={String(run.run_id ?? i)}><span>{String(run.run_type ?? "run")}<small>{run.started_at ? new Date(String(run.started_at)).toLocaleString("es-CO") : ""}</small></span><span className={run.status === "succeeded" ? "goodText" : run.status === "failed" ? "warn" : ""}>{String(run.status ?? "—")}</span></div>) : <p className="muted">Aún no hay ejecuciones registradas.</p>}</div></Panel></section>
