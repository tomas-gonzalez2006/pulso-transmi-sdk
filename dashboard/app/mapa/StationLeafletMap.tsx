...
export default function StationLeafletMap({ stations, predictions, selectedId, onSelect }: { stations: Station[]; predictions: Prediction[]; selectedId?: string; onSelect: (station: Station) => void }) {
  const node = useRef<HTMLDivElement>(null); const mapRef = useRef<LeafletMap | null>(null); const layerRef = useRef<import("leaflet").LayerGroup | null>(null);
  useEffect(() => {
    let disposed = false;
  ...
  }, [stations, predictions, selectedId, onSelect]);
