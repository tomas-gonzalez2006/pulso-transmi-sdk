...
def main() -> None:
    ...
    with httpx.Client(base_url=BASE_URL, headers=headers, timeout=60, follow_redirects=True) as client:
        ...
        cycle_stations = {str(target["station_id"]) for target in cycle["targets"]}
        history_is_complete = history_covers_cycle(history, cutoff, cycle_stations)
        if not history_is_complete:
            if history is not None:
                print(
                    "Supabase history is incomplete for the cycle; "
